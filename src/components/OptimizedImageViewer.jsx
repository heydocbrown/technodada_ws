...
export const OptimizedImageViewer = memo(function ImageViewer({ 
  imageUrl, 
  startReveal, 
  syncDuration,
  revealMode = 'top-down' 
    ...
    if (startReveal && imageLoaded && syncDuration) {
      ...
      const animate = (currentTime) => {
        ...
        if (corruption.level > 0.3) {
          // Chaotic easing
          easedProgress = progress + Math.sin(progress * Math.PI * 4) * corruption.level * 0.1;
        }
  ...
  }, [startReveal, imageLoaded, syncDuration, corruption.level, reportPerformanceImpact]);
  ...
  const imageStyle = {
    ...
    transform: shouldGlitch && Math.random() < 0.1 ? `scale(${1 + Math.random() * 0.1})` : 'scale(1)',
    filter: corruption.level > 0.7 ? `contrast(${1 + corruption.level}) saturate(${1 + corruption.level})` : 'none',
  };
