...
function OverlayViewer({ prompt, imageUrl, startOverlay, revealMode = 'topToBottom' }) {
    ...
    useEffect(() => {
        if (revealMode === 'randomPixels' && imageLoaded && imageAspectRatio) {
            const maskUrl = generateRandomPixelMask(400, Math.floor(400 / imageAspectRatio), revealProgress);
    ...
    const handleImageLoad = (e) => {
        ...
        if (revealMode === 'randomPixels') {
            const maskUrl = generateRandomPixelMask(400, Math.floor(400 / (e.target.naturalWidth / e.target.naturalHeight)), 0);
            setMaskDataUrl(maskUrl);
        }
    };
                ...
                <div className="overlay-viewer">
                    <div className="overlay-container" style={{
                        ...
                        <div className="overlay-text">
                            &gt; <span dangerouslySetInnerHTML={{__html: prompt?.replace(/\n/g, '<br/>') || ''}} />
                        </div>
