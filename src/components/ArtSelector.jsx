  ...
  useEffect(() => {
    if (selectedProject && artData.length > 0) {
      ...
      const filtered = artData.filter(
        item => item.artproject === selectedProject && item.on_website === 'yes',
      );
    ...
    if (selectedId) {
      ...
      if (selectedArt) {
        ...
        const artWithPrompt = {
          ...selectedArt,
          cleaned_prompt:
            selectedArt.cleaned_prompt ||
            selectedArt.prompt ||
            selectedArt.human_prompt ||
            'VOID.NULL contemplates existence',
        };
        onArtSelect(artWithPrompt);
      } else {
        // Artistic error handling - reality has diverged
        console.log(
          'Selected art not found in quantum state. Reality.status = UNCERTAIN',
        );
      }
    }
  };
    ...
    <div className="art-selector-section">
      ...
      {selectedProject && (
        <div className="dropdown-container">
          <select className="art-dropdown" onChange={handleArtSelect} defaultValue="">
            <option value="">
