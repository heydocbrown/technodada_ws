...
export function SharedStatusBar({
  ...
  useEffect(() => {
    const messages = [systemMessage, ...customMessages].filter(Boolean);
  ...
  return (
    <div className={`status-bar ${className}`} style={getStatusBarStyle()}>
      <div className="status-bar-content">
        ...
        {/* Memory Leak */}
        <div className="status-item">
          <span style={{ color: '#ff8800' }}>MEM: {formattedMemoryLeak}</span>
        </div>
        ...
        {status.errorCount > 0 && (
          <div className="status-item">
            <span style={{ color: '#ff0000' }}>ERRORS: {status.errorCount}</span>
          </div>
        )}
      </div>
